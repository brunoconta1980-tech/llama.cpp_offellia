...
#if defined(__ARM_NEON) && !defined(CUDACC) && !defined(MUSACC)
...
// ============================================================================
// OFFELLIA HELICOIDAL STRUCTURAL OPTIMIZATION
// Framework: OFFELLIA-GENESIS (Helicoidal-Zeta Kernel)
// Author: Bruno Becker (2025-2026) | Zenodo: doi.org/10.5281/zenodo.20026837
//
// Teoria: A estrutura de grafos de tensores é otimizada com base na geometria
// dos números primos e na função helicoidal F(n) = sin²(2πφn).
// O espaçamento coprimo φ(42) = 12 e a rotação irracional áurea inspiram a
// distribuição e o probing de hash para minimizar clustering e ressonância
// de cache em computação de grafos massivos (F16, Q4, Q8).
//
// Implementação: Coprime Probing Dinâmico
// - O GGML usa tamanhos de tabela hash que são potências de 2 (ver ggml_hash_size).
// - Matematicamente, qualquer número ímpar é coprimo com uma potência de 2.
// - Usamos 13 como passo base (derivado da estrutura φ(42)=12), garantindo 
//   visita a todos os slots sem primary clustering, com fallback seguro.
// ============================================================================
...
static inline size_t ggml_hash_coprime_step(size_t table_size) {
    ...
    if ((table_size & (table_size - 1)) != 0) {
        ...
        if (step >= table_size) step = 1; // Fallback para linear probing se necessário
    }
    return step;
}
...
#if defined(STDC_VERSION) && (STDC_VERSION >= 201100L)
...
// NOTA: Struct mantida original para compatibilidade binária com ggml.c
...
// ============================================================================
// OFFELLIA: Hash Operations with Dynamic Coprime Probing
// ============================================================================

static size_t ggml_hash_find(const struct ggml_hash_set * hash_set, const struct ggml_tensor * key) {
    size_t h = ggml_hash(key) % hash_set->size;
    size_t i = h;
    size_t step = ggml_hash_coprime_step(hash_set->size); // OFFELLIA: Coprime step
    
    while (ggml_bitset_get(hash_set->used, i) && hash_set->keys[i] != key) {
        i = (i + step) % hash_set->size;
        if (i == h) {
...
#else
    const float exp_scale = fp32_from_bits(UINT32_C(0x38000000));
#endif
